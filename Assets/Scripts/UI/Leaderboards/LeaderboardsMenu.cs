...
public class LeaderboardsMenu : MonoBehaviour
{
    ...
    private void OnDestroy()
    {
        ...
        {
            clienChatText.onEndEdit.RemoveListener(
                OnChatInputSubmitted);
        }
        ...
        if (OnlineManager.Instance != null)
        {
            ...
            OnlineManager.Instance.Chat.OnlinePlayersUpdated -=
    ...
    private void Start()
    {
        ...
        clienChatText.onEndEdit.AddListener(
            OnChatInputSubmitted);
    ...
    public void GlobalScrollUp()
    {
        globalChatScrollRect.verticalNormalizedPosition =
            Mathf.Clamp01(globalChatScrollRect.verticalNormalizedPosition + step);
    }
    ...
    {
        playerScrollRect.verticalNormalizedPosition =
            Mathf.Clamp01(playerScrollRect.verticalNormalizedPosition + step);
    }
    ...
    private void OnChatInputSubmitted(
    string message)
    {
        if (!Input.GetKeyDown(KeyCode.Return) &&
            !Input.GetKeyDown(KeyCode.KeypadEnter))
        ...
        string message)
    {
        ...
        await OnlineManager.Instance.Chat.SendChat(
            message);
    ...
    {
        Debug.Log(
            $"[LeaderboardsMenu] RecentMessagesReceived fired. " +
    ...
    {
        Debug.Log(
            "[LeaderboardsMenu] RefreshChatHistory() called."
        );

        if (globalChatText == null)
        {
            Debug.LogWarning(
                "[LeaderboardsMenu] globalChatText is null."
            );
        ...
        {
            Debug.LogWarning(
                "[LeaderboardsMenu] OnlineManager is null."
            );
        ...
        {
            Debug.LogWarning(
                "[LeaderboardsMenu] ChatManager is null."
            );
        ...
        Debug.Log(
            $"[LeaderboardsMenu] Cached history count = " +
            $"{messages.Count}"
        );
        ...
        foreach (ChatMessage message in messages)
        {
            ...
            {
                string displayName =
                    string.IsNullOrEmpty(message.Status)
                        ? message.Username
    ...
    private async UniTask RefreshChatTextSize()
    {
        ...
        if (globalChatText == null ||
            globalChatScrollRect == null ||
            globalChatContent == null)
            return;
        ...
        globalChatContent.SetSizeWithCurrentAnchors(
