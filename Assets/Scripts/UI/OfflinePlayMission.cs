...
public class OfflinePlayMission : PlayMissionManager
{
    ...
    {
        if (statisticsWindow) statisticsWindow.SetActive(false);
    ...
    {
        if (expertButton) expertButton.SetActive(selectedGame == Game.platinum);
        if (customButton) customButton.SetActive(selectedGame == Game.gold);
        if (achievementsButton) achievementsButton.gameObject.SetActive(selectedGame == Game.platinum);

        if (statisticsButton)
            statisticsButton.gameObject.SetActive(currentlySelectedType != Type.custom);
    }
    ...
    protected override void UpdateMissionSpecificUI(int levelIndex)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                sb.AppendLine($"{Utils.FormatTime(time)}<sprite name=\"gold\">");
            }
            else if (time != -1 && (selectedGame == Game.platinum || (selectedGame == Game.gold && currentlySelectedType == Type.custom)))
            {
                if (time < MissionInfo.instance.ultimateTime)
                    sb.AppendLine($"{Utils.FormatTime(time)}<sprite name=\"ultimate\">");
                else if (time < MissionInfo.instance.goldTime)
                    sb.AppendLine($"{Utils.FormatTime(time)}<sprite name=\"platinum\">");
                else
        ...
        if (bestTimesText) bestTimesText.text = sb.ToString();
    }
    ...
    protected override void HandleEmptyMissionList()
    {
        ...
        if (levelDescriptionText) levelDescriptionText.gameObject.SetActive(false);
        if (levelImage) levelImage.color = Color.clear;
        if (currentLevelText) currentLevelText.text = "Level 0";

        if (notQualifiedImage) notQualifiedImage.SetActive(true);
        if (notQualifiedText) notQualifiedText.SetActive(true);

        if (prev) prev.interactable = false;
        if (next) next.interactable = false;
        if (play) play.interactable = false;
