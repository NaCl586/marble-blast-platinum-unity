...
public class AchievementManager : MonoBehaviour
{
    ...
    private void CacheAllMissions()
    {
        ...
        if (MissionInfo.instance == null) return;
    ...
    async void InitLBAchiev()
    {
        ...
        if (HasTime("Moto-Marblecross", 5f) &&
    ...
    {
        foreach (
            KeyValuePair<string, MyRankResponse> pair
            in userLeaderboardData)
        {
            MyRankResponse data =
                pair.Value;
    ...
    private async UniTask LoadUserLeaderboardData()
    {
        ...
        {
            if (mission == null ||
                string.IsNullOrWhiteSpace(mission.levelName))
            {
                continue;
            }
            ...
            try
            {
                ...
                if (response != null)
                {
                    userLeaderboardData[mission.levelName] =
                        response;
                }
            }
            ...
            {
                Debug.LogWarning(
                    $"Failed to load leaderboard data for " +
                    $"{leaderboardLevel}: {ex.Message}");
            }
        }
    }
    ...
    private bool HasBeatenGoldTime(string levelName)
    {
        ...
        float time = GetPlayerBestTime(levelName);
        return time != -1 && time < mission.goldTime;
    }
    ...
    void InitAchiev()
    {
        ...
        if (totalBeginnerPlatinum >= 25) achievImage[0].sprite = achievSprite[0];
        if (totalIntermediatePlatinum >= 35) achievImage[1].sprite = achievSprite[1];
        if (totalAdvancedPlatinum >= 35) achievImage[2].sprite = achievSprite[2];
        if (totalExpertPlatinum >= 25) achievImage[3].sprite = achievSprite[3];
        if (platinumTimes >= 120) achievImage[4].sprite = achievSprite[4];
        if (ultimateTimes >= 120) achievImage[5].sprite = achievSprite[5];
        if (totalEasterEgg >= 1) achievImage[6].sprite = achievSprite[6];
        if (totalEasterEgg >= 120) achievImage[7].sprite = achievSprite[7];
    }
    ...
    {
        if (pages == null || pages.Length == 0 || loadingText.activeSelf) return;
