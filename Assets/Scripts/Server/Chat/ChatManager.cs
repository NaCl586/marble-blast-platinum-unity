...
namespace Server
{
    public class ChatManager
    {
        ...
        private readonly List<ChatMessage> recentMessages =
        ...
        public event Action<IReadOnlyList<OnlinePlayer>>
            OnlinePlayersUpdated;

        public event Action<IReadOnlyList<ChatMessage>>
        ...
        private string ChatUrl =>
            $"{serverConfig.BaseUrl.TrimEnd('/')}{ChatHubPath}";
        ...
        public async UniTask<bool> Connect(string accessToken)
        {
            ...
            {
                Debug.LogError(
                    "Cannot connect to chat: access token is empty."
                );
            ...
            if (connection != null)
            {
                ...
                {
                    Debug.Log(
                        "Chat connection is already active."
                    );
            ...
            {
                Debug.Log(
                    $"Connecting to MarbleServer chat: " +
                    $"{ChatUrl}"
                );
                ...
                Debug.Log(
                    "Connected to MarbleServer chat."
                );
            ...
            catch (Exception exception)
            {
                ...
                Debug.LogError(
                    $"Failed to connect to MarbleServer chat:\n" +
                    $"{exception}"
                );
            ...
            Exception error)
        {
            ...
            {
                Debug.LogError(
                    $"Chat connection permanently closed:\n" +
                    $"{error}"
                );
            }
            else
            ...
            Debug.LogError(
                "Unable to reconnect to MarbleServer chat."
            );
            ...
            string message)
        {
            ...
            {
                Debug.LogWarning(
                    "Cannot send chat: not connected."
                );
        ...
        public IReadOnlyList<ChatMessage>
            GetRecentMessages()
        {
        ...
        private void OnRecentMessages(
            ChatMessage[] messages)
        {
            Debug.Log(
                $"[ChatManager] RecentMessages received. " +
            ...
            Debug.Log(
                $"[ChatManager] Cached history count = " +
                $"{recentMessages.Count}"
            );

            RecentMessagesReceived?.Invoke(
                recentMessages
            );
        }
        ...
        public IReadOnlyList<OnlinePlayer>
            GetOnlinePlayers()
        {
            ...
            foreach (
                KeyValuePair<string, string> player
                in onlinePlayers)
            {
                result.Add(
                    new OnlinePlayer
        ...
        public async UniTask SetStatus(
            string status)
        {
            if (connection == null)
                return;
        ...
        public async UniTask Disconnect()
        {
            ...
            try
            {
                    ...
                    HubConnectionState.Disconnected)
                {
                    ...
                    Debug.Log(
                        "Disconnected from MarbleServer chat."
                    );
                }
            }
            catch (Exception exception)
            {
                Debug.LogError(
                    $"Failed to disconnect from chat:\n" +
                    $"{exception}"
                );
            }
        ...
        private async UniTask DisposeConnection()
        {
            ...
            {
                Debug.LogError(
                    $"Failed to dispose chat connection:\n" +
                    $"{exception}"
                );
            }
